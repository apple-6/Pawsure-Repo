...
class _OwnerInboxState extends State<OwnerInbox> with SingleTickerProviderStateMixin {
  ...
  List<OwnerInboxItem> get _confirmedBookings => 
      _allBookings.where((b) => b.status.toLowerCase() != 'pending').toList();
  ...
  Future<void> _loadBookings() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _allBookings = bookingsJson.map((json) => OwnerInboxItem.fromJson(json)).toList();
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _handleCancel(int bookingId) async {
    try {
      ...
      Get.snackbar('Success', 'Request cancelled.', backgroundColor: Colors.grey, colorText: Colors.white);
      _loadBookings(); 
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: Colors.white,
        elevation: 0,
        leading: const BackButton(color: Colors.black), // Back button to return to Community/Feed
        actions: [
          IconButton(
              ...
              : TabBarView(
                  ...
                  children: [
                    ...
                    _confirmedBookings.isEmpty
                        ? const Center(child: Text('No upcoming bookings', style: TextStyle(color: Colors.grey)))
                        : ListView.builder(
                            padding: const EdgeInsets.all(16),
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text("Sitter: ${inbox.sitterName}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      Text("For: ${inbox.petName}", style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                    ],
                  ),
                ),
                Container(
            ...
            Row(
              ...
              children: [
                ...
                Text("RM${inbox.estimatedCost.toStringAsFixed(0)}", style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1CCA5B))),
              ],
            ),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text("Sitter: ${inbox.sitterName}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      Text("Pet: ${inbox.petName}", style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                    ],
                  ),
                ),
                Container(
